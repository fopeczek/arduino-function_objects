...
//  - Simple to use - use FunctionObject<> as the function argument
...
template<typename>
struct FunctionObject; // intentionally not defined
...
struct FunctionObject<R(Args...)>
{
    ...
    // Dispatch() is instantiated by the FunctionObject constructor,
    ...
    template<typename T>
    FunctionObject(T&& target)
            : m_Dispatcher(&Dispatch<T>) //typename std::decay<T>::type
            , m_Target(&target)
    {
    }
    ...
    FunctionObject(TargetFunctionRef target)
